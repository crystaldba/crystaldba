...
async def import_chatgpt_data(file: Path) -> None:
    ...
    def output_progress(imported_count: int, total_count: int, message_count: int) -> Text:
                ...
                for _message_id, message_data in chat_data["mapping"].items():
                    ...
                    if message_info:
                        ...
                        if metadata:
                            ...
                            chat.model = "gpt-4-turbo" if model == "gpt-4" else "gpt-3.5-turbo"
                            session.add(chat)
                        ...
                        message = MessageDao(
                            ...
                            timestamp=datetime.fromtimestamp(message_info.get("create_time", 0) or 0),
                            model=model,
                            meta=metadata,
                        )
                        session.add(message)
                        message_count += 1
                        live.update(output_progress(chat_number, len(data), message_count))
