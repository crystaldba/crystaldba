...
# Process CRYSTAL_API_URL if it contains localhost
if [[ -n "$CRYSTAL_API_URL" && "$CRYSTAL_API_URL" == *"localhost"* ]]; then
    echo "Found localhost in CRYSTAL_API_URL: $CRYSTAL_API_URL" >&2
    new_url=$(replace_localhost "$CRYSTAL_API_URL")
    if [[ $? -eq 0 ]]; then
        export CRYSTAL_API_URL="$new_url"
        echo "Updated CRYSTAL_API_URL: $CRYSTAL_API_URL" >&2
    else
        echo "Keeping original CRYSTAL_API_URL: $CRYSTAL_API_URL" >&2
    fi
fi
...
# First argument is python, second is -m, third is tui.__main__
processed_args+=("$1")
processed_args+=("$2")
processed_args+=("$3")
shift 3
...
# Debug: print the command about to be executed
echo "----------------" >&2
...
if [ $exit_code -ne 0 ]; then
    ...
    echo "CRYSTAL_API_URL=$CRYSTAL_API_URL" >&2
fi
